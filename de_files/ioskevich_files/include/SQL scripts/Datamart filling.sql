...
INSERT INTO datamart."опыт_сотрудника_в_предметных_обла" 
	("User ID", 
	"Предметные области", 
	"Дата", 
...
DELETE FROM datamart."опыт_сотрудника_в_предметных_обла"
USING (SELECT "User ID",
	        "Предметные области",
	        "Дата",
	         count(*) as "Счетчик дублей"
	   FROM datamart."опыт_сотрудника_в_предметных_обла"
	   GROUP BY "User ID",
	        "Предметные области",
	        "Дата"
	   HAVING count(*) > 1) as t
WHERE "опыт_сотрудника_в_предметных_обла"."User ID" = t."User ID" AND
        "опыт_сотрудника_в_предметных_обла"."Предметные области" = t."Предметные области" AND
        "опыт_сотрудника_в_предметных_обла"."Дата" = t."Дата" AND
        "Уровень знаний в предметной облас" = 'Junior';
